...
extends BlockContainer {
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, Block block) {
        ...
        } else if (LOTRBlockBeacon.isLit((IBlockAccess)world, i, j, k) && world.getBlock(i, j + 1, k).getMaterial() == Material.water) {
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
        ...
        if (itemstack != null && itemstack.getItem() == Items.water_bucket && LOTRBlockBeacon.isLit((IBlockAccess)world, i, j, k)) {
            world.playSoundEffect((double)i + 0.5, (double)j + 0.5, (double)k + 0.5, "random.fizz", 0.5f, 2.6f + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.8f);
            if (!entityplayer.capabilities.isCreativeMode) {
    ...
    public void onEntityCollidedWithBlock(World world, int i, int j, int k, Entity entity) {
        if (entity.isBurning() && !LOTRBlockBeacon.isLit((IBlockAccess)world, i, j, k) && world.getBlock(i, j + 1, k).getMaterial() != Material.water) {
            world.playSoundEffect((double)i + 0.5, (double)j + 0.5, (double)k + 0.5, "fire.ignite", 1.0f, world.rand.nextFloat() * 0.4f + 0.8f);
            if (!world.isRemote) {
                LOTRBlockBeacon.setLit(world, i, j, k, true);
                entity.setDead();
    ...
    public void randomDisplayTick(World world, int i, int j, int k, Random random) {
        if (!LOTRBlockBeacon.isLit((IBlockAccess)world, i, j, k)) {
            return;
        }
