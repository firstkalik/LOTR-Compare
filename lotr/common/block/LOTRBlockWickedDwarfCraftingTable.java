...
public class LOTRBlockWickedDwarfCraftingTable
extends LOTRBlockCraftingTable {
    @SideOnly(value=Side.CLIENT)
    private IIcon[] tableIcons;

    public LOTRBlockWickedDwarfCraftingTable() {
        super(Material.rock, LOTRFaction.MORDOR, 69);
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
        ...
        boolean hasRequiredAlignment = LOTRLevelData.getData(entityplayer).getAlignment(this.tableFaction) >= 10.0f;
        boolean bl = hasRequiredAlignment2 = LOTRLevelData.getData(entityplayer).getAlignment(LOTRFaction.GUNDABAD) >= 10.0f;
        if (hasRequiredAlignment || hasRequiredAlignment2) {
            ...
            if (!world.isRemote) {
                LOTRAlignmentValues.notifyAlignmentNotHighEnough(entityplayer, 10.0f, this.tableFaction, LOTRFaction.GUNDABAD);
            }
        }
        return true;
    }
