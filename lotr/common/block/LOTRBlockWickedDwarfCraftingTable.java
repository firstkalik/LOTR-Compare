...
extends LOTRBlockCraftingTable {
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
        ...
        boolean bl = hasRequiredAlignment2 = LOTRLevelData.getData(entityplayer).getAlignment(LOTRFaction.GUNDABAD) >= 100.0f;
        if (hasRequiredAlignment || hasRequiredAlignment2) {
            ...
            if (!world.isRemote) {
                LOTRAlignmentValues.notifyAlignmentNotHighEnough(entityplayer, 100.0f, this.tableFaction, LOTRFaction.GUNDABAD);
            }
        }
        return true;
    }
