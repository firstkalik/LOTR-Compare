...
public class LOTRBlockDarkElfCraftingTable
extends LOTRBlockCraftingTable {
    @SideOnly(value=Side.CLIENT)
    private IIcon[] tableIcons;

    public LOTRBlockDarkElfCraftingTable() {
        super(Material.rock, LOTRFaction.UTUMNO, 71);
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
        ...
        boolean bl = hasRequiredAlignment2 = LOTRLevelData.getData(entityplayer).getAlignment(LOTRFaction.MORDOR) >= 10.0f;
        if (hasRequiredAlignment || hasRequiredAlignment2) {
            ...
            if (!world.isRemote) {
                LOTRAlignmentValues.notifyAlignmentNotHighEnough(entityplayer, 1.0f, this.tableFaction, LOTRFaction.MORDOR);
            }
        }
        return true;
    }
