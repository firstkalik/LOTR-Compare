...
public class LOTRBlockrottenfleshBlock
extends LOTRModBlock {
    public LOTRBlockrottenfleshBlock(int lightLevel) {
        super(LOTRModBlock.materialMagma1);
        this.setHardness(3.0f);
        this.setResistance(5.0f);
    }

    public LOTRBlockrottenfleshBlock() {
        this(3);
    }
    ...
    public void onEntityCollidedWithBlock(World world, int i, int j, int k, Entity entity) {
        ...
        if (!world.isRemote && entity instanceof EntityLivingBase && this.isEntityVulnerable(living = (EntityLivingBase)entity)) {
            int dur = 100;
            living.addPotionEffect(new PotionEffect(Potion.confusion.id, dur * 2));
        }
    }
