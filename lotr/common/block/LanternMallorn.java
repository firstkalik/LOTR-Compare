...
extends Block {
    ...
    protected static boolean canLanternStayAt(World world, int x, int y, int z) {
        ...
        if (meta == 1) {
            return world.getBlock(x, y + 1, z).isSideSolid((IBlockAccess)world, x, y + 1, z, ForgeDirection.DOWN) || LanternMallorn.canSustainLanternOnBottom(world, x, y + 1, z);
        }
        return world.getBlock(x, y - 1, z).isSideSolid((IBlockAccess)world, x, y - 1, z, ForgeDirection.UP) || LanternMallorn.canSustainLanternOnTop(world, x, y - 1, z);
    }

    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        return world.getBlock(x, y + 1, z).isSideSolid((IBlockAccess)world, x, y + 1, z, ForgeDirection.DOWN) || LanternMallorn.canSustainLanternOnBottom(world, x, y + 1, z) || world.getBlock(x, y - 1, z).isSideSolid((IBlockAccess)world, x, y - 1, z, ForgeDirection.UP) || LanternMallorn.canSustainLanternOnTop(world, x, y - 1, z);
    }

    public int onBlockPlaced(World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int meta) {
        if (world.getBlock(x, y + 1, z).isSideSolid((IBlockAccess)world, x, y + 1, z, ForgeDirection.DOWN) || LanternMallorn.canSustainLanternOnBottom(world, x, y + 1, z)) {
            if (side == 0) {
                return 1;
            }
            if (!world.getBlock(x, y - 1, z).isSideSolid((IBlockAccess)world, x, y - 1, z, ForgeDirection.UP) && !LanternMallorn.canSustainLanternOnTop(world, x, y - 1, z)) {
                return 1;
            }
        }
        return 0;
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block neighborBlock) {
        if (!LanternMallorn.canLanternStayAt(world, x, y, z)) {
            if (!world.isRemote) {
