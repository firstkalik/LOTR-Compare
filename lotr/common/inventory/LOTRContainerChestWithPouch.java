...
extends ContainerChest {
    ...
    public LOTRContainerChestWithPouch(EntityPlayer entityplayer, int pouchSlot, IInventory chest) {
        ...
        for (int j = 0; j < this.numPouchRows; ++j) {
            for (i = 0; i < 9; ++i) {
                ...
                Slot slot = this.pouchContainer.getSlotFromInventory((IInventory)this.pouchContainer.pouchInventory, pouchSlotID);
        ...
        for (i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.addSlotToContainer(new Slot((IInventory)entityplayer.inventory, j + i * 9 + 9, 8 + j * 18, playerSlotsY + i * 18));
            }
        }
        for (i = 0; i < 9; ++i) {
            this.addSlotToContainer(new Slot((IInventory)entityplayer.inventory, i, 8 + i * 18, playerSlotsY + 58));
        }
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer entityplayer, int i) {
        ...
        Slot aPouchSlot = this.getSlotFromInventory((IInventory)this.pouchContainer.pouchInventory, 0);
