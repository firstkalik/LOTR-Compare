...
public class LOTREntityAIOrcAvoidGoodPlayer2
extends EntityAIBase {
    ...
    public LOTREntityAIOrcAvoidGoodPlayer2(LOTREntityOrc2 orc, float f, double d) {
        this.theOrc = orc;
    ...
    public boolean shouldExecute() {
        ...
        List validPlayers = this.theOrc.worldObj.selectEntitiesWithinAABB(EntityPlayer.class, this.theOrc.boundingBox.expand((double)this.distanceFromEntity, (double)this.distanceFromEntity / 2.0, (double)this.distanceFromEntity), new IEntitySelector(){
            ...
            public boolean isEntityApplicable(Entity entity) {
                ...
                if (entityplayer.capabilities.isCreativeMode || LOTREntityAIOrcAvoidGoodPlayer2.access$000((LOTREntityAIOrcAvoidGoodPlayer2)LOTREntityAIOrcAvoidGoodPlayer2.this).currentRevengeTarget == entityplayer) {
                    return false;
                }
                float alignment = LOTRLevelData.getData(entityplayer).getAlignment(LOTREntityAIOrcAvoidGoodPlayer2.this.theOrc.getFaction());
    ...
    private boolean anyNearbyOrcsAttacked() {
        List nearbyAllies = this.theOrc.worldObj.selectEntitiesWithinAABB(EntityLiving.class, this.theOrc.boundingBox.expand((double)this.distanceFromEntity, (double)this.distanceFromEntity / 2.0, (double)this.distanceFromEntity), new IEntitySelector(){
            ...
            public boolean isEntityApplicable(Entity entity) {
                if (entity != LOTREntityAIOrcAvoidGoodPlayer2.this.theOrc) {
                    return LOTRMod.getNPCFaction(entity).isGoodRelation(LOTREntityAIOrcAvoidGoodPlayer2.this.theOrc.getFaction());
                }
                return false;
            }
        });
