...
public class LOTREntityThrownRock2
extends EntityThrowable {
    private int rockRotation;
    private float damage;

    public LOTREntityThrownRock2(World world) {
        super(world);
        this.setSize(4.0f, 4.0f);
    }

    public LOTREntityThrownRock2(World world, EntityLivingBase entityliving) {
        super(world, entityliving);
        this.setSize(4.0f, 4.0f);
    }

    public LOTREntityThrownRock2(World world, double d, double d1, double d2) {
        super(world, d, d1, d2);
        this.setSize(4.0f, 4.0f);
    }
            ...
            if (flag) {
                if (this.getSpawnsTroll()) {
                    ...
                    if (this.rand.nextInt(3) == 0) {
                        troll = new LOTREntityMountainSnowTroll(this.worldObj);
                    }
                ...
                for (int l = 0; l < drops; ++l) {
                    this.dropItem(Item.getItemFromBlock((Block)Blocks.air), 1);
                }
    ...
    public void handleHealthUpdate(byte b) {
        if (b == 15) {
            for (int l = 0; l < 32; ++l) {
                LOTRMod.proxy.spawnParticle("chill", this.posX + this.rand.nextGaussian() * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height, this.posZ + this.rand.nextGaussian() * (double)this.width, 0.0, 0.0, 0.0);
            }
        } else {
