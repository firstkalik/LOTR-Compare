...
public class LOTREntityArrowAvari
extends EntityArrow
implements IEntityAdditionalSpawnData {
    protected boolean isInGround = false;

    public LOTREntityArrowAvari(World world) {
        super(world);
    }

    public LOTREntityArrowAvari(World world, double d, double d1, double d2) {
        super(world, d, d1, d2);
    }

    public LOTREntityArrowAvari(World world, EntityLivingBase shooter, EntityLivingBase target, float charge, float inaccuracy) {
        super(world, shooter, target, charge, inaccuracy);
    }

    public LOTREntityArrowAvari(World world, EntityLivingBase shooter, float charge) {
    ...
    public void onCollideWithPlayer(EntityPlayer entityplayer) {
        ...
        if (!this.worldObj.isRemote && isInGround && this.arrowShake <= 0) {
            ...
            if (this.canBePickedUp == 1 && !entityplayer.inventory.addItemStackToInventory(new ItemStack(LOTRMod.arrowAvari, 1))) {
                pickup = false;
            }
            if (pickup) {
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.isRemote && !this.isInGround) {
            int c = Potion.waterBreathing.getLiquidColor();
