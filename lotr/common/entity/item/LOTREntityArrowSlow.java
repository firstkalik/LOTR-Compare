...
public class LOTREntityArrowSlow
extends EntityArrow
implements IEntityAdditionalSpawnData {
    protected boolean isInGround = false;

    public LOTREntityArrowSlow(World world) {
        super(world);
    }

    public LOTREntityArrowSlow(World world, double d, double d1, double d2) {
        super(world, d, d1, d2);
    }

    public LOTREntityArrowSlow(World world, EntityLivingBase shooter, EntityLivingBase target, float charge, float inaccuracy) {
        super(world, shooter, target, charge, inaccuracy);
    }

    public LOTREntityArrowSlow(World world, EntityLivingBase shooter, float charge) {
    ...
    public void onCollideWithPlayer(EntityPlayer entityplayer) {
        ...
        if (!this.worldObj.isRemote && isInGround && this.arrowShake <= 0) {
            ...
            if (this.canBePickedUp == 1 && !entityplayer.inventory.addItemStackToInventory(new ItemStack(LOTRMod.arrowSlow, 1))) {
                pickup = false;
            }
            if (pickup) {
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.isRemote && !this.isInGround) {
            int c = Potion.moveSlowdown.getLiquidColor();
            double d0 = (double)(c >> 16 & 0xFF) / 255.0;
            double d1 = (double)(c >> 8 & 0xFF) / 255.0;
            double d2 = (double)(c & 0xFF) / 255.0;
            for (int i = 0; i < 4; ++i) {
                this.worldObj.spawnParticle("mobSpell", this.posX + this.motionX / 4.0, this.posY + this.motionY / 4.0, this.posZ + this.motionZ / 4.0, d0, d1, d2);
            }
        }
    }
