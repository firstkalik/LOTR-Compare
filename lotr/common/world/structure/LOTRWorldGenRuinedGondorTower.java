    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        if (this.restrictions) {
            for (i1 = i - 3; i1 <= i + 3; ++i1) {
                for (k1 = k + 3; k1 <= k + 3; ++k1) {
                    ...
                    if (block == Blocks.grass || block.isWood((IBlockAccess)world, i1, j1 - 1, k1) || block.isLeaves((IBlockAccess)world, i1, j1 - 1, k1)) continue;
                    return false;
                }
            }
        }
        for (i1 = i - 3; i1 <= i + 3; ++i1) {
            for (k1 = k - 3; k1 <= k + 3; ++k1) {
                ...
                if (Math.abs(i1 - i) >= 3 || Math.abs(k1 - k) >= 3 || random.nextInt(20) == 0) continue;
                this.setBlockAndNotifyAdequately(world, i1, j + 5, k1, Blocks.planks, 0);
            }
        }
        ...
        for (k12 = k; k12 <= k + 2; ++k12) {
            this.setBlockAndNotifyAdequately(world, i - 2, j + 6, k12, LOTRMod.slabSingle, 10);
        }
        if (random.nextBoolean()) {
        ...
        for (i1 = i - 4; i1 <= i + 4; ++i1) {
            for (k1 = k - 4; k1 <= k + 4; ++k1) {
                ...
                if ((Math.abs(i1 - i) != 4 || Math.abs(k1 - k) % 2 != 0) && (Math.abs(k1 - k) != 4 || Math.abs(i1 - i) % 2 != 0) || !LOTRMod.isOpaque(world, i1, j + 9, k1)) continue;
                this.placeRandomBrick(world, random, i1, j + 10, k1);
            }
        }
        for (j12 = j + 1; j12 <= j + 9; ++j12) {
            if (rotation == 2) {
                if (!LOTRMod.isOpaque(world, i + 3, j12, k)) continue;
                this.setBlockAndNotifyAdequately(world, i + 2, j12, k, Blocks.ladder, 4);
                continue;
            }
            if (!LOTRMod.isOpaque(world, i, j12, k + 3)) continue;
            this.setBlockAndNotifyAdequately(world, i, j12, k + 2, Blocks.ladder, 2);
        }
        if (rotation == 0) {
            for (j12 = j + 1; j12 <= j + 2; ++j12) {
                this.setBlockAndNotifyAdequately(world, i - 1, j12, k - 3, LOTRMod.brick, 5);
                this.setBlockAndNotifyAdequately(world, i, j12, k - 3, Blocks.air, 0);
                this.setBlockAndNotifyAdequately(world, i + 1, j12, k - 3, LOTRMod.brick, 5);
            }
            for (k12 = k - 4; k12 >= k - 7; --k12) {
                for (i12 = i - 2; i12 <= i + 2; i12 += 4) {
                    j1 = world.getHeightValue(i12, k12);
                    id = world.getBlock(i12, j1 - 1, k12);
                    if (id != Blocks.grass || random.nextInt(4) == 0) continue;
                    this.setBlockAndNotifyAdequately(world, i12, j1, k12, LOTRMod.wall, 3 + random.nextInt(3));
                }
            }
        }
        if (rotation == 1) {
            for (j12 = j + 1; j12 <= j + 2; ++j12) {
                this.setBlockAndNotifyAdequately(world, i + 3, j12, k - 1, LOTRMod.brick, 5);
                this.setBlockAndNotifyAdequately(world, i + 3, j12, k, Blocks.air, 0);
                this.setBlockAndNotifyAdequately(world, i + 3, j12, k + 1, LOTRMod.brick, 5);
            }
            for (i1 = i + 4; i1 <= i + 7; ++i1) {
                for (k1 = k - 2; k1 <= k + 2; k1 += 4) {
                    j1 = world.getHeightValue(i1, k1);
                    id = world.getBlock(i1, j1 - 1, k1);
                    if (id != Blocks.grass || random.nextInt(4) == 0) continue;
                    this.setBlockAndNotifyAdequately(world, i1, j1, k1, LOTRMod.wall, 3 + random.nextInt(3));
                }
            }
        }
        if (rotation == 2) {
