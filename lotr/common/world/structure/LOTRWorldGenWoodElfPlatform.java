...
public class LOTRWorldGenWoodElfPlatform
    ...
    private boolean generateFacingSouth(World world, Random random, int i, int j, int k) {
        ...
        if (this.restrictions) {
            for (i1 = i - 2; i1 <= i + 2; ++i1) {
                for (j1 = j; j1 <= j + 4; ++j1) {
                    if (!world.getBlock(i1, j1, k + 1).isWood((IBlockAccess)world, i1, j1, k + 1)) {
                        return false;
                    }
                    for (k1 = k; k1 >= k - 3; --k1) {
    ...
    private boolean generateFacingWest(World world, Random random, int i, int j, int k) {
        ...
        if (this.restrictions) {
            for (k1 = k - 2; k1 <= k + 2; ++k1) {
                for (j1 = j; j1 <= j + 4; ++j1) {
                    if (!world.getBlock(i - 1, j1, k1).isWood((IBlockAccess)world, i - 1, j1, k1)) {
                        return false;
                    }
                    for (i1 = i; i1 <= i + 3; ++i1) {
                        if (world.isAirBlock(i1, j1, k1)) continue;
