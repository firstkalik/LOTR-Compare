...
public class LOTRWorldGenRohanBarrow
    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        for (i12 = i - radius; i12 <= i + radius; ++i12) {
            for (int j13 = j + height; j13 >= j; --j13) {
                for (k1 = k - radius; k1 <= k + radius; ++k1) {
                    ...
                    boolean grass = !LOTRMod.isOpaque(world, i12, j13 + 1, k1);
                    this.setBlockAndNotifyAdequately(world, i12, j13, k1, (Block)(grass ? Blocks.grass : Blocks.dirt), 0);
        ...
        for (i12 = i - radius; i12 <= i + radius; ++i12) {
            for (k12 = k - radius; k12 <= k + radius; ++k12) {
                for (j1 = j - 1; !LOTRMod.isOpaque(world, i12, j1, k12) && j1 >= 0; --j1) {
                    i2 = i12 - i;
                    int k2 = k12 - k;
                    if (i2 * i2 + k2 * k2 > radius * radius) continue;
                    this.setBlockAndNotifyAdequately(world, i12, j1, k12, Blocks.dirt, 0);
        ...
        for (int l = 0; l < 12; ++l) {
            ...
            if (world.getBlock(i1, (j14 = world.getHeightValue(i1, k1 = k - random.nextInt(radius) + random.nextInt(radius))) - 1, k1) != Blocks.grass) continue;
            this.setBlockAndNotifyAdequately(world, i1, j14, k1, LOTRMod.simbelmyne, 0);
        }
        j += height;
        for (i12 = i - 1; i12 < i + 1; ++i12) {
            for (k12 = k - 1; k12 <= k + 1; ++k12) {
                this.setBlockAndNotifyAdequately(world, i12, j - 1, k12, Blocks.air, 0);
            }
        }
        for (i12 = i - 2; i12 <= i + 2; ++i12) {
            for (k12 = k - 2; k12 <= k + 2; ++k12) {
                for (j1 = j - 2; j1 >= j - 4; --j1) {
                    this.setBlockAndNotifyAdequately(world, i12, j1, k12, Blocks.air, 0);
                }
                this.setBlockAndNotifyAdequately(world, i12, j - 5, k12, LOTRMod.slabDouble2, 1);
            }
        }
        for (int j15 = j - 3; j15 >= j - 4; --j15) {
            for (i1 = i - 4; i1 <= i + 4; ++i1) {
                for (k1 = k - 1; k1 <= k + 1; ++k1) {
                    this.setBlockAndNotifyAdequately(world, i1, j15, k1, Blocks.air, 0);
                    if (Math.abs(i1 - i) <= 2) continue;
                    this.setBlockAndNotifyAdequately(world, i1, j - 5, k1, LOTRMod.brick, 4);
                }
            }
