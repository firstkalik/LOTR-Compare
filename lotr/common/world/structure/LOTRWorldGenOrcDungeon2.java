...
public class LOTRWorldGenOrcDungeon2
extends LOTRWorldGenStructureBase {
    public LOTRWorldGenOrcDungeon2(boolean flag) {
        super(flag);
    }
    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        if (height >= 1 && height <= 5) {
            ...
            int orcs = MathHelper.getRandomIntegerInRange((Random)random, (int)3, (int)6);
            while (orcs > 0) {
                LOTREntityOrc orc;
                ...
                if ((orc = (LOTREntityOrc)EntityList.createEntityByName((String)LOTREntities.getStringFromClass(orcClass), (World)world)).isOrcBombardier()) continue;
                orc.setLocationAndAngles((double)i + 0.5, (double)(j + 1), (double)k + 0.5, 0.0f, 0.0f);
