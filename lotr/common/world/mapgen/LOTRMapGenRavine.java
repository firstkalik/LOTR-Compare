...
extends MapGenRavine {
    ...
    protected void func_151540_a(long seed, int chunkX, int chunkZ, Block[] blocks, double d, double d1, double d2, float f, float ravineAngle, float f2, int intPar1, int intPar2, double increase) {
        ...
        while (intPar1 < intPar2) {
            double d6 = 1.5 + (double)(MathHelper.sin((float)((float)intPar1 * 3.1415927f / (float)intPar2)) * f * 1.0f);
            double d7 = d6 * increase;
            d6 *= (double)random.nextFloat() * 0.25 + 0.75;
            d7 *= (double)random.nextFloat() * 0.25 + 0.75;
            float f6 = MathHelper.cos((float)f2);
            float f7 = MathHelper.sin((float)f2);
            d += (double)(MathHelper.cos((float)ravineAngle) * f6);
            d1 += (double)f7;
            d2 += (double)(MathHelper.sin((float)ravineAngle) * f6);
            f2 *= 0.7f;
            f2 += f4 * 0.05f;
            ...
            if (flag || random.nextInt(4) != 0) {
                ...
                if (d >= chunkCentreX - 16.0 - d6 * 2.0 && d2 >= chunkCentreZ - 16.0 - d6 * 2.0 && d <= chunkCentreX + 16.0 + d6 * 2.0 && d2 <= chunkCentreZ + 16.0 + d6 * 2.0) {
                    int i1;
                    int xMin = MathHelper.floor_double((double)(d - d6)) - chunkX * 16 - 1;
                    int xMax = MathHelper.floor_double((double)(d + d6)) - chunkX * 16 + 1;
                    int yMin = MathHelper.floor_double((double)(d1 - d7)) - 1;
                    int yMax = MathHelper.floor_double((double)(d1 + d7)) + 1;
                    int zMin = MathHelper.floor_double((double)(d2 - d6)) - chunkZ * 16 - 1;
                    int zMax = MathHelper.floor_double((double)(d2 + d6)) - chunkZ * 16 + 1;
