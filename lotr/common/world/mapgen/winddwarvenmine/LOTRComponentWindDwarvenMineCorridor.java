...
public class LOTRComponentWindDwarvenMineCorridor
extends StructureComponent {
    private int sectionCount;
    private boolean ruined;

    public LOTRComponentWindDwarvenMineCorridor() {
    }

    public LOTRComponentWindDwarvenMineCorridor(int i, Random random, StructureBoundingBox structureBoundingBox, int j, boolean r) {
        super(i);
        ...
        for (int l = 0; l < this.sectionCount; ++l) {
            ...
            for (int i : new int[]{0, 2}) {
                ...
                for (int j = 0; j <= wallHeight; ++j) {
                    this.placeBlockAtCurrentPosition(world, LOTRMod.wallStone9, 0, i, j, k, structureBoundingBox);
                }
            }
            for (int j = 0; j <= 2; ++j) {
            ...
            this.placeBlockAtCurrentPosition(world, LOTRMod.brick9, 12, 1, -1, k, structureBoundingBox);
            ...
            if (random.nextInt(120) == 0) {
                this.generateStructureChestContents(world, structureBoundingBox, random, 2, 0, k - 1, LOTRChestContents.LOTRChestContents2.WIND_DWARVEN_MINE_CORRIDOR.items, LOTRChestContents.getRandomItemAmount(LOTRChestContents.LOTRChestContents2.BLUE_DWARVEN_MINE_CORRIDOR, random));
            }
            if (random.nextInt(120) != 0) continue;
            this.generateStructureChestContents(world, structureBoundingBox, random, 0, 0, k + 1, LOTRChestContents.LOTRChestContents2.WIND_DWARVEN_MINE_CORRIDOR.items, LOTRChestContents.getRandomItemAmount(LOTRChestContents.LOTRChestContents2.BLUE_DWARVEN_MINE_CORRIDOR, random));
        }
