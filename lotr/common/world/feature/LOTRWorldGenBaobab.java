...
extends WorldGenAbstractTree {
    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
                ...
                for (k1 = k - trunkCircleWidth - 1; k1 <= k + trunkCircleWidth + 1 && flag; ++k1) {
                    ...
                    for (j1 = j; j1 <= j + 1 + height; ++j1) {
                        if (j1 >= 0 && j1 < 256) {
                            ...
                            if (this.isReplaceable(world, i1, j1, k1) || block.isReplaceable((IBlockAccess)world, i1, j1, k1)) continue;
                            flag = false;
                            continue;
                        }
                        flag = false;
        ...
        for (j12 = j + height - 1; j12 > j + (int)((float)height * 0.75f); --j12) {
            int branches = 2 + random.nextInt(3);
            for (l = 0; l < branches; ++l) {
                float angle = random.nextFloat() * 3.1415927f * 2.0f;
                int i13 = i;
                int k13 = k;
                int j2 = j12;
                int length = MathHelper.getRandomIntegerInRange((Random)random, (int)4, (int)6);
                for (int l1 = trunkCircleWidth; l1 < trunkCircleWidth + length && this.isReplaceable(world, i13 = i + (int)(1.5f + MathHelper.cos((float)angle) * (float)l1), j2 = j12 - 3 + l1 / 2, k13 = k + (int)(1.5f + MathHelper.sin((float)angle) * (float)l1)); ++l1) {
