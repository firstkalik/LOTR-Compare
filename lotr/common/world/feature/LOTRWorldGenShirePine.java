                ...
                for (int j1 = j + height; j1 >= minLeafHeight; --j1) {
                    for (int i1 = i - leafWidth; i1 <= i + leafWidth; ++i1) {
                        for (int k1 = k - leafWidth; k1 <= k + leafWidth; ++k1) {
                            ...
                            int i2 = i1 - i;
                            int k2 = k1 - k;
                            if (leafWidth > 0 && Math.abs(i2) == leafWidth && Math.abs(k2) == leafWidth || !(block = world.getBlock(i1, j1, k1)).isReplaceable((IBlockAccess)world, i1, j1, k1) && !block.isLeaves((IBlockAccess)world, i1, j1, k1)) continue;
