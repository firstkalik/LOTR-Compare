...
public class LOTRWorldGenBigTrees
extends WorldGenAbstractTree {
    ...
    private void genTreeLayer(int par1, int par2, int par3, float par4, byte par5, Block par6, int meta) {
        ...
        for (int j1 = -i1; j1 <= i1; ++j1) {
            ...
            while (k1 <= i1) {
                ...
                if (block.getMaterial() != Material.air && !block.isLeaves((IBlockAccess)this.worldObj, aint1[0], aint1[1], aint1[2])) {
                    ++k1;
                    continue;
                }
    ...
    private void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, Block block, int meta) {
        ...
        if (aint2[b1] != 0) {
            ...
            for (int j = 0; j != k; j += b4) {
                int i1;
                aint3[b1] = MathHelper.floor_double((double)((double)(par1ArrayOfInteger[b1] + j) + 0.5));
                aint3[b2] = MathHelper.floor_double((double)((double)par1ArrayOfInteger[b2] + (double)j * d0 + 0.5));
                aint3[b3] = MathHelper.floor_double((double)((double)par1ArrayOfInteger[b3] + (double)j * d1 + 0.5));
                int b5 = 0;
    ...
    private int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger) {
        ...
        for (i = 0; i != j; i += b4) {
            ...
            if (block.getMaterial() != Material.air && !block.isLeaves((IBlockAccess)this.worldObj, aint3[0], aint3[1], aint3[2])) break;
        }
