    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        for (j1 = leafMin; j1 <= leafTop; ++j1) {
            ...
            for (int b = 0; b < branches; ++b) {
                ...
                for (int l = 0; l < (length *= this.extraTrunk + 1) && Math.abs(i2 - i13) <= leafWidth && Math.abs(k2 - k13) <= leafWidth && ((block = world.getBlock(i2, j1, k2)).isReplaceable((IBlockAccess)world, i2, j1, k2) || block.isLeaves((IBlockAccess)world, i2, j1, k2)); ++l) {
