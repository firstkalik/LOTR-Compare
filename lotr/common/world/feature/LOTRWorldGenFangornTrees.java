...
extends WorldGenAbstractTree {
    ...
    public boolean generate(World world, Random random, int i, int j, int k) {
        ...
        for (int j1 = 0; j1 < height; ++j1) {
            ...
            for (int i1 = i - width; i1 <= i + width; ++i1) {
                for (int k1 = k - width; k1 <= k + width; ++k1) {
                    ...
                    if (block == Blocks.air || block.isLeaves((IBlockAccess)world, i1, j + j1, k1)) {
        ...
        int angle = 0;
        while (angle < 360) {
            float angleR = (float)(angle += 10 + random.nextInt(20)) / 180.0f * 3.1415927f;
            ...
            int boughBaseHeight = j + MathHelper.floor_double((double)((float)height * (0.9f + random.nextFloat() * 0.1f)));
                    ...
                    for (int i3 = i2 - leafRange; i3 <= i2 + leafRange; ++i3) {
                        for (int j3 = j2 - leafRange; j3 <= j2 + leafRange; ++j3) {
                            for (int k3 = k2 - leafRange; k3 <= k2 + leafRange; ++k3) {
                                ...
                                if (dist >= (leafRange - 1) * (leafRange - 1) && (dist >= leafRange * leafRange || random.nextInt(3) == 0) || (block2 = world.getBlock(i3, j3, k3)).getMaterial() != Material.air && !block2.isLeaves((IBlockAccess)world, i3, j3, k3)) continue;
