...
public enum LOTRMountains {
    ...
    BREE(LOTRWaypoint.BREE.getX() + 3.0, LOTRWaypoint.BREE.getY(), 2.0f, 500),
    ...
    MOUNT_RED4(LOTRWaypoint.RED_SOUTH.getX(), LOTRWaypoint.RED_SOUTH.getY(), 2.5f, 250),
    MOUNT_RED3(LOTRWaypoint.RED_MIDDLE.getX(), LOTRWaypoint.RED_MIDDLE.getY(), 2.5f, 250),
    MOUNT_RED2(LOTRWaypoint.RED_NORTH.getX(), LOTRWaypoint.RED_NORTH.getY(), 2.5f, 250),
    MOUNT_RED7(LOTRWaypoint.SNOW_BAD.getX(), LOTRWaypoint.SNOW_BAD.getY(), 2.5f, 250),
    MOUNT_RED8(LOTRWaypoint.WIND_POINT.getX(), LOTRWaypoint.WIND_POINT.getY(), 2.5f, 250),
    MOUNT_RED9(LOTRWaypoint.GUNUD_DUM.getX(), LOTRWaypoint.GUNUD_DUM.getY(), 2.5f, 250),
    MOUNT_RED10(LOTRWaypoint.SHATR_ZIGIL.getX(), LOTRWaypoint.SHATR_ZIGIL.getY(), 2.5f, 250),
    MOUNT_RED5(LOTRWaypoint.SNOW_PICK2.getX(), LOTRWaypoint.SNOW_PICK2.getY(), 2.5f, 250);
    ...
    public float getHeightBoost(int x, int z) {
        ...
        if (distSq < rangeSq) {
            if (this.lavaRange > 0 && distSq < (double)(this.lavaRange * this.lavaRange)) {
    ...
    public static int getLavaHeight(int x, int z) {
        for (LOTRMountains m : LOTRMountains.values()) {
            ...
            double d2;
            double dz;
            if (m.lavaRange <= 0 || d * (dx = (double)(x - m.xCoord)) + d2 * (dz = (double)(z - m.zCoord)) >= (double)((m.lavaRange + 6) * (m.lavaRange + 6))) continue;
