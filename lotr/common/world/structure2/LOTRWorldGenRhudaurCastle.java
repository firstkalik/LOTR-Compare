...
public class LOTRWorldGenRhudaurCastle
extends LOTRWorldGenStructureBase2 {
    ...
    public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
        ...
        int width = MathHelper.getRandomIntegerInRange((Random)random, (int)6, (int)15);
        int height = MathHelper.getRandomIntegerInRange((Random)random, (int)3, (int)8);
        for (int i1 = -width; i1 <= width; ++i1) {
            for (int k1 = -width; k1 <= width; ++k1) {
                ...
                if (i2 == width || k2 == width) {
                    int j12;
                    float f = MathHelper.randomFloatClamp((Random)random, (float)0.7f, (float)1.0f);
                    int h = Math.round((float)height * f);
                    int b = 1;
                    if (k1 == -width && i2 <= 1) {
                        b = 4;
                    }
                    if (IntMath.mod((int)(i2 + k2), (int)2) == IntMath.mod((int)width, (int)2)) {
                        ++h;
                    }
                    ...
                    for (int j2 = b; j2 <= h; ++j2) {
                        ...
                        boolean cracked;
                        int j3 = j12 + j2;
                        boolean bl = low = j2 < (int)((float)height * 0.5f) && j2 < h;
