...
public class LOTRWorldGenRuinedRedDwarvenTower
extends LOTRWorldGenStructureBase2 {
    ...
    protected LOTRItemBanner.BannerType bannerType = LOTRItemBanner.BannerType.STIFFBEARDS;
    protected LOTRChestContents chestContents = LOTRChestContents.LOTRChestContents2.STIFFBEARDS;
    protected boolean ruined = false;

    public LOTRWorldGenRuinedRedDwarvenTower(boolean flag) {
        super(flag);
    }

    protected LOTREntityNPC getCommanderNPC(World world) {
        return new LOTREntityStiffbeardCap(world);
    }
    ...
    public boolean generateWithSetRotation(World world, Random random, int i, int j, int k, int rotation) {
        ...
        int sections = 8 + random.nextInt(3);
        ...
        for (int l = 0; l <= sections; ++l) {
            ...
            LOTREntityStiffbeardWarrior dwarf = random.nextInt(3) == 0 ? new LOTREntityStiffbeardCrossbow(world) : new LOTREntityStiffbeardWarrior(world);
    ...
    private ItemStack[] getDisplayArmorOrNull(World world, Random random) {
        LOTREntityStiffbeardWarrior armorGuard = new LOTREntityStiffbeardWarrior(world);
    ...
    private void placeRandomFeature(World world, Random random, int i, int j, int k, int randomFeature, boolean flip) {
        if (randomFeature == 0) {
            ...
            this.setBlockAndMetadata(world, i, j, k, this.plankSlabBlock, this.plankSlabMeta | 8);
            if (!this.ruined) {
                this.placeBarrel(world, random, i, j + 1, k, flip ? 3 : 2, LOTRFoods.REDDWARF_DRINK);
            }
        }
    }
}
