...
public class LOTRItemSwordKeranKessertin
extends LOTRItemSword
implements LOTRStoryItem {
    protected float lotrWeaponDamage;
    private boolean isHoldingSword = false;

    public LOTRItemSwordKeranKessertin(LOTRMaterial AULE) {
        super(LOTRMaterial.UTUMNO_LEGENDARY);
        this.setMaxDamage(4800);
        this.lotrWeaponDamage = 9.5f;
        LOTRWeaponStats.registerMeleeSpeed(LOTRItemSwordKeranKessertin.class, 1.0f);
        LOTRWeaponStats.registerMeleeReach(LOTRItemSwordKeranKessertin.class, 1.0f);
        this.setCreativeTab((CreativeTabs)LOTRCreativeTabs.tabStory);
    }

    @Override
    public LOTRItemSwordKeranKessertin addWeaponDamage(float f) {
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
        ...
        if (!world.isRemote && world.rand.nextFloat() <= 0.3f && !target.isPotionActive(Potion.wither)) {
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int itemSlot, boolean isSelected) {
        if (entity instanceof EntityPlayer) {
            ...
            if (this.isHoldingSword && player.isPotionActive(Potion.wither)) {
