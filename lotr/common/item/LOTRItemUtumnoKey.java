...
extends Item {
    ...
    public static ItemStack getRandomKeyPart(Random rand) {
        ...
        itemstack.setItemDamage(MathHelper.getRandomIntegerInRange((Random)rand, (int)2, (int)(keyTypes.length - 1)));
        return itemstack;
    }
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int side, float f, float f1, float f2) {
        if (LOTRDimension.getCurrentDimensionWithFallback(world) == LOTRDimension.UTUMNO && side == 1) {
            ...
            if (utumnoLevel == keyUsageLevel && j < utumnoLevel.corridorBaseLevels[0] && block.isOpaqueCube()) {
                if (!world.isRemote) {
                    world.playSoundEffect((double)i + 0.5, (double)j + 0.5, (double)k + 0.5, "random.explode", 2.0f, 0.2f + world.rand.nextFloat() * 0.2f);
                }
                for (int l = 0; l < 60; ++l) {
                    world.spawnParticle("blockcrack_" + Block.getIdFromBlock((Block)block) + "_" + meta, (double)i + 0.5 + world.rand.nextGaussian() * 1.0, (double)j + 1.5, (double)k + 0.5 + world.rand.nextGaussian() * 1.0, 0.0, 0.0, 0.0);
                }
                if (!world.isRemote) {
                    ...
                    int stairZ = k - 1;
                    int stairY = j;
                    while (!(stairY < targetLevel.corridorBaseLevels[0] || LOTRUtumnoLevel.forY(stairY) == targetLevel && world.isAirBlock(stairX, stairY + 1, stairZ) && World.doesBlockHaveSolidTopSurface((IBlockAccess)world, (int)stairX, (int)stairY, (int)stairZ))) {
