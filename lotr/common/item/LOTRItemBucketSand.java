...
public class LOTRItemBucketSand
extends ItemBucket {
    public LOTRItemBucketSand(Block containedBlock) {
    ...
    public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        ...
        boolean bl = isSnowdrift = clickedBlock == LOTRMod.quicksand;
        if (isSnowdrift) {
            int newX = x;
            int newY = y;
            int newZ = z;
            ...
            if (world.isAirBlock(newX, newY, newZ) || world.getBlock(newX, newY, newZ).isReplaceable((IBlockAccess)world, newX, newY, newZ)) {
                if (!world.isRemote) {
                    world.setBlock(newX, newY, newZ, LOTRMod.quicksand);
        ...
        if (isVegetation || canReplace) {
            if (!world.isRemote) {
                ...
                world.setBlock(x, y, z, LOTRMod.quicksand);
                if (!player.capabilities.isCreativeMode) {
                    --itemStack.stackSize;
                    if (itemStack.stackSize <= 0) {
                        player.inventory.setInventorySlotContents(player.inventory.currentItem, new ItemStack(Items.bucket));
