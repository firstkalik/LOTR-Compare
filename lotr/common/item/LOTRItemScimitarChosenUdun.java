...
public class LOTRItemScimitarChosenUdun
extends LOTRItemSword
implements LOTRStoryItem {
    protected float lotrWeaponDamage;
    private boolean isHoldingSword = false;

    public LOTRItemScimitarChosenUdun(LOTRMaterial AULE) {
        super(LOTRMaterial.UTUMNO_LEGENDARY);
        this.setMaxDamage(4800);
        this.lotrWeaponDamage = 9.5f;
        LOTRWeaponStats.registerMeleeSpeed(LOTRItemScimitarChosenUdun.class, 1.0f);
        LOTRWeaponStats.registerMeleeReach(LOTRItemScimitarChosenUdun.class, 1.0f);
        this.setCreativeTab((CreativeTabs)LOTRCreativeTabs.tabStory);
    }

    @Override
    public LOTRItemScimitarChosenUdun addWeaponDamage(float f) {
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
        ...
        if (!world.isRemote && world.rand.nextFloat() <= 0.3f) {
            ...
            int duration = baseDuration + world.rand.nextInt(baseDuration);
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int itemSlot, boolean isSelected) {
        if (entity instanceof EntityPlayer) {
            ...
            if (this.isHoldingSword && player.isBurning()) {
                player.extinguish();
            }
        }
    }
