...
public class LOTRModelRohanMarshalHelmet
extends LOTRModelBiped {
    ...
    public LOTRModelRohanMarshalHelmet(float f) {
        ...
        for (int i = 0; i < this.manes.length; ++i) {
            ModelRenderer mane;
            this.manes[i] = mane = new ModelRenderer((ModelBase)this, 32, 0);
    ...
    public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, Entity entity) {
        ...
        for (int i = 0; i < this.manes.length; ++i) {
            ...
            mane.rotateAngleX = (mid - Math.abs((float)i - mid)) / mid * 0.22f;
            mane.rotateAngleY = ((float)i - mid) / mid * 0.17f;
            mane.rotateAngleX += MathHelper.sin((float)(f * 0.4f)) * f1 * 0.2f;
        }
    }
}
