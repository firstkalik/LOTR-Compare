...
extends ModelBase {
    ...
    public LOTRModelBeacon() {
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 4; ++j) {
                this.logs[i][j] = new ModelRenderer((ModelBase)this, 30, 15);
                this.logs[i][j].addBox(-1.5f, 0.0f, -7.0f, 3, 3, 14);
                this.logs[i][j].setRotationPoint(i != 1 ? -6.0f + (float)j * 4.0f : 0.0f, 17.0f - (float)i * 3.0f, i == 1 ? -6.0f + (float)j * 4.0f : 0.0f);
                if (i != 1) continue;
